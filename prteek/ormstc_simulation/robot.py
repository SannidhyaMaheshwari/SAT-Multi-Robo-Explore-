...
class Robot:
    ...
    def move_towards_start(self):
        """Move towards starting position by backtracking along spanning tree"""
        if (self.x, self.y) == (self.start_x, self.start_y):
            return False

        # Use spanning tree to backtrack - follow parent edges
        current_pos = (self.x, self.y)

        if current_pos in self.spanning_tree:
            # Follow the spanning tree parent edge
            parent_x, parent_y = self.spanning_tree[current_pos]
            self.x, self.y = parent_x, parent_y
            return True
        else:
            # Not in spanning tree (shouldn't happen for properly explored robots)
            # Fall back to direct movement as last resort
    ...
    def step(self):
        ...
        # Check if globally complete first - if so, mark as completed immediately
        if not self.completed and self.is_globally_complete():
            self.completed = True
            self.comm_system.broadcast_message(
                self.robot_id,
                "completed",
        ...
        if next_x is not None and next_y is not None:
            ...
            if self.backtrack():
                ...
                # No more moves available - check if completed
