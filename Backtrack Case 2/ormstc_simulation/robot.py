...
class Robot:
    def __init__(self, robot_id, start_x, start_y, grid, comm_system):
        ...
        # --- Intersection rollback state (Step 1 scaffold) ---
        self.flush_window_start_idx = 0  # index in self.path where the post-last-flush window begins
        self.rollback_target = None      # (x,y) target when rolling back to pre-intersection cell
        ...
        self.intersection_cells = []    # cells (since last flush) that overlapped and must be purged
        self.purging_intersection = False
    ...
    def check_failed_robots(self):
        ...
        for robot_id, connection_list in self.connections.items():
            if self.comm_system.robot_states.get(robot_id, {}).get("completed"):
                continue  # don’t “take over” a robot that explicitly finished
    ...
    def process_messages(self):
        ...
        for message in messages:
            if message.msg_type == MessageType.POSITION_BROADCAST.value:
                ...
                self.comm_system.update_robot_state(message.sender_id, {"completed": True})

            elif message.msg_type == "takeover":
                # Another robot is taking over a failed robot's area
                pass

            elif message.msg_type == "unclaim":  # or MessageType.UNCLAIM
                sender = message.sender_id
                cells = message.data.get("cells", [])
                for (cx, cy) in cells:
                    # Only remove if our local view attributes this cell to the sender
                    if self.local_map.get_tree_owner(cx, cy) == sender:
                        self.local_map.remove_tree_edge(cx, cy)
    ...
    def step(self):
        ...
        # If we're intentionally holding (Step 1/2), do nothing this tick
        if getattr(self, "stop_in_place", False) and not getattr(self, "flush_replan_pending", False):
            return False
        ...
        # Keep global coverage view fresh before any planning/backtracking decisions
        self.update_global_coverage(None)   # arg is unused inside; None is fine
        # --- Step 3: replan from the rollback target after purge ---
        if getattr(self, "flush_replan_pending", False):
            # Consume the latch and allow movement
            self.flush_replan_pending = False
            self.stop_in_place = False

            # Refresh local perception at the target node
            self.sense_environment()

            # Try to resume exploration from this exact node
            nx, ny = self.find_next_cell()
            if nx is not None and ny is not None:
                # Resume forward coverage immediately
                return self.move_to_cell(nx, ny)

            # No immediate frontier here — do standard DFS fallback
            if self.backtrack():
                return True

            # Handle completion cases (same semantics as your normal tail)
            if (self.x, self.y) == (self.start_x, self.start_y) and self.check_completion():
                ...
                self.comm_system.broadcast_message(
                    ...
                    {"robot_id": self.robot_id, "reason": "local_done_after_rollback_replan"}
                )
                return False
        ...
        if not self.comm_system.is_malicious(self.robot_id):
            if self.comm_system.flush_epoch > self.last_seen_flush_epoch:
                ...
                suppress_now = self.suppress_backtrack_once
                self.suppress_backtrack_once = False

                # Cells we visited/moved through since previous flush anchor
                start_idx = getattr(self, "flush_window_start_idx", 0)
                recent_path = self.path[start_idx:] if start_idx < len(self.path) else []

                # Find earliest overlapped cell index in recent_path
                earliest_idx = None
                for i2, c2 in enumerate(recent_path):
                    if c2 in flushed_set:
                        earliest_idx = i2
                        break

                if earliest_idx is not None and not suppress_now:
                    # --- NEW: gather ALL intersected cells in the window (purge union of runs)
                    self.intersection_cells = [c for c in recent_path if c in flushed_set]

                    # Compute rollback target = cell BEFORE first intersected cell
                    if earliest_idx > 0:
                        self.rollback_target = self.path[start_idx + earliest_idx - 1]
                    else:
                        self.rollback_target = self.path[start_idx - 1] if start_idx > 0 else self.path[0]

                    # Enter purge + directed rollback
                    self.purging_intersection = True            # NEW
                    self.rollback_in_progress = True
                    self.backtracking = True
                    self.replans_after_overlap += 1
                    self.overlap_candidates.clear()

                    # Anchor next flush window to end-of-path as of now
                    self.flush_window_start_idx = len(self.path)
                    return False

                # No actionable overlap → anchor and move on
                self.flush_window_start_idx = len(self.path)
                self.overlap_candidates.clear()

        # --- Directed rollback to pre-intersection cell (Step 2 adds purge) ---
        if getattr(self, "rollback_in_progress", False):
            # If we have arrived at the rollback target
            if self.rollback_target is not None and (self.x, self.y) == self.rollback_target:
                # Exit rollback; remain stopped here (Step 3 will replan)
                self.rollback_in_progress = False
                self.backtracking = False
                self.purging_intersection = False         # NEW: purge is finished by now
                self.flush_replan_pending = True 
                self.stop_in_place = True
                return False

            # --- while walking upward, purge intersected cells and notify peers; then step up ---
            purged_here = False
            if self.purging_intersection and self.intersection_cells:
                here = (self.x, self.y)
                if here in self.intersection_cells:
                    # 1) Remove our local claim/edge for this cell (only if we own it)
                    owner = self.local_map.get_tree_owner(*here)
                    if owner == self.robot_id:
                        self.local_map.remove_tree_edge(*here)

                    # 2) Broadcast UNCLAIM so peers also drop our claim on this cell
                    payload_cells = [here]
                    if hasattr(self.comm_system, "broadcast_unclaim"):
                        self.comm_system.broadcast_unclaim(self.robot_id, payload_cells)
                    else:
                        self.comm_system.broadcast_message(self.robot_id, "unclaim", {"cells": payload_cells})

                    # 3) Mark purged locally
                    purged_here = True
                    self.intersection_cells.remove(here)

            prev = (self.x, self.y)
            if self.backtrack():
                if purged_here:
                    # After moving up, it's safe to drop our own DFS edge for the just-left cell
                    self.spanning_tree.pop(prev, None)
                return True

            # Step one parent upward toward rollback_target
            # if self.backtrack():
            #     return True

            # If no parent (degenerate), just stop to avoid thrash
            self.rollback_in_progress = False
            self.backtracking = False
            self.purging_intersection = False
            self.flush_replan_pending = True   # <-- add this
            self.stop_in_place = True
            return False
